...
export const isBlocked = (date) => {
    ...
    if (config.blocked[year]) {
        if (config.blocked[year][month]) {
            if (
                config.blocked[year][month].findIndex((el) => el === day) !== -1
            ) {
                return true
            }
        }
...
export const isBooked = (date) => {
    ...
    if (config.booked[year]) {
        if (config.booked[year][month]) {
            if (
                config.booked[year][month].findIndex((el) => el === day) !== -1
            ) {
                return true
            }
        }
...
export const getDatesBetweenDates = (startDate, endDate) => {
    let dates = []
    //don't modify original date
    const theDate = new Date(startDate)
    theDate.setDate(theDate.getDate() + 1) //excludes first date
